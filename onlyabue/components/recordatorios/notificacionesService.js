...
export async function primerRecordatorio(medicamentoData) {
  const { usuarioId, medicamentoId, intervalo, horaInicial, cantidad, nombre  } = medicamentoData;
  ...
  for (let i = 1; i < cantidad; i++) {
    nextNotificationTime = new Date(nextNotificationTime.getTime() + intervalo * 60 * 1000);//minutos -> cambiar a horas
